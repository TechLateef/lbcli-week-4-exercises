# Create a raw transaction and add this message in it: "btrust builder 2026"

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
transaction=""

rawtx=$(bitcoin-cli -regtest decoderawtransaction $transaction)
TXID=$(echo $rawtx | jq -r '.txid')
vout1=$(echo $rawtx | jq -r '.vout[0].n')
vout2=$(echo $rawtx | jq -r '.vout[1].n')
recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
data=$(echo -n "btrust builder 2026" | xxd -p)

bitcoin-cli -regtest createrawtransaction "[{\"txid\":\"$TXID\",\"vout\":$vout1},{\"txid\":\"$TXID\",\"vout\":$vout2}]" "{\"data\":\"$data\",\"$recipient\":0.2}"